// --- PraiseBook Manager - Lógica JS (Con Motor ChordPro Integrado) ---
...
function conectarApp() {
    ...
    peer.on('open', (id) => {
        ...
        // 🚨 QUITAMOS serialization: 'json' PARA EVITAR EL BUG
        conn = peer.connect(peerIdDeLaApp);
        
        conn.on('open', () => {
            actualizarEstado('Conectado a la App', 'good');
        ...
        conn.on('data', (rawData) => {
            // 🚨 TRADUCIMOS EL TEXTO A CÓDIGO SIEMPRE
            let data = rawData;
        ...
        conn.on('error', () => { actualizarEstado('Error', 'bad'); });
    });

    peer.on('error', (err) => {
        actualizarEstado('Error de conexión', 'bad');
...
function actualizarEstado(mensaje, tipo) {
    ...
    el.status.className = '';
    if (tipo === 'good') el.status.classList.add('status-connected');
    else if (tipo === 'bad') el.status.classList.add('status-disconnected');
...
function actualizarListaCanciones() {
    const filtro = el.searchInput.value.toLowerCase();
    ...
    songsData.forEach((cancion, index) => {
        ...
        // Usamos el "index" numérico en lugar del _id
        if (index === currentSongId) item.classList.add('active'); 
        
        let icono = cancion.isVisual ? ' 🖼️' : '';
        item.innerText = tituloSeguro + icono;
        
        item.onclick = () => seleccionarCancion(index); // Le pasamos el número de posición
...
function seleccionarCancion(index) {
    ...
    if (!cancion) return;
    currentSongId = index; // Guardamos el número de posición
...
function guardarCambios() {
    ...
    const cancionActualizada = {
        indiceBaseDatos: esNueva ? null : currentSongId, // Enviamos el número exacto
        title: titulo,
        artist: el.editor.artist.value.trim().toUpperCase(),
        lyrics: el.editor.content.value,
        tone: cancionOriginal.tone || "--",
        bpm: cancionOriginal.bpm || "",
        velLetra: cancionOriginal.velLetra || "",
...
function dibujarVisualizador() {
    ...
    const cancionActual = songsData.find(c => c._id === currentSongId);
    ...
    if (cancionActual && cancionActual.isVisual) {
        el.preview.innerHTML = `<div class="empty-preview" style="color: #4CAF50;"><h3>${title}</h3><p>Esto es un archivo ${cancionActual.tipoArchivo.toUpperCase()}. Se ve directo en el celular.</p></div>`;
        return;
    }
...
function subirTono() { alert("Usa los ajustes de tono directamente en el visor del celular."); }
function bajarTono() { alert("Usa los ajustes de tono directamente en el visor del celular."); }